...
namespace pierresilva\AccessControl\Traits;

trait PermissionTrait
{
    /**
     * The access control cache tag used by the model.
    ...
    {
        return $this->belongsToMany('\pierresilva\AccessControl\Models\Permission')->withTimestamps();
    }
    ...
    public function getPermissions()
    {
        ...
        $cacheKey   = 'pierresilva.'.substr(static::getAccessControlTag(), 0, -1).'.permissions.'.$primaryKey;

        if (method_exists(app()->make('cache')->getStore(), 'tags')) {
            return app()->make('cache')->tags(static::getAccessControlTag())->remember($cacheKey, 60, function () {
                return $this->getFreshPermissions();
            });
        } else {
            return app()->make('cache')->remember($cacheKey, 60, function () {
                return $this->getFreshPermissions();
            });
        }
    ...
    public function flushPermissionCache(array $tags = null)
    {
        ...
        if (method_exists(app()->make('cache')->getStore(), 'tags')) {
            if ($tags === null) {
                $tags = [ static::getAccessControlTag() ];
            }
            ...
            app()->make('cache')->forget($cacheKey);
        }
    }
